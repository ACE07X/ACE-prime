  ...
  async addLabels(taskId: string, labels: string[]): Promise<ServiceResult<Task>> {
    try {
      const { data: task } = await db
        ...
        .select('labels')
        .eq('id', taskId)
        .single() as { data: { labels: string[] } | null };
  ...
  async getProjectStats(projectId: string): Promise<ServiceResult<{
    ...
    try {
      const { data, error } = await db
        ...
        .eq('project_id', projectId) as { data: Array<{ status: string; priority: string; assignee_id: string | null; due_date: string | null }> | null; error: any };
